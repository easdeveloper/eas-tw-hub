(() => {
    ...
    // Shared URL sanitizer for diagnostic metadata; never retain arbitrary query/fragment data.
    const sanitizeUrl = (value) => { try { const url = new URL(value, location.href); const safeUrl = new URL(url.origin + url.pathname); for (const key of ['screen', 'mode']) { const value = url.searchParams.get(key); if (value && /^[a-z_]{1,40}$/.test(value)) safeUrl.searchParams.set(key, value); } const village = url.searchParams.get('village'); if (/^\d+$/.test(village || '')) safeUrl.searchParams.set('village', village); return safeUrl.href; } catch { return '[invalid-url]'; } };
    const pageContext = () => { try { const url = new URL(location.href); return { url: sanitizeUrl(url.href), screen: url.searchParams.get('screen'), mode: url.searchParams.get('mode'), try: url.searchParams.get('try'), villageId: String(window.game_data?.village?.id || url.searchParams.get('village') || '') }; } catch { return {}; } };
    ...
    const write = (level, module, action, data = {}, context = {}) => {
        ...
        if (EAS.Logger) EAS.Logger[level](module, action, { ...entry.data, executionId: entry.executionId, error: entry.error });
        else { const entries = storageGet(LOG_KEY, []); entries.push(entry); storageSet(LOG_KEY, entries.slice(-LOG_LIMIT)); }
        const method = level === 'error' ? 'error' : level === 'warn' ? 'warn' : level === 'info' ? 'info' : 'debug'; console[method](`[EAS ${module}] ${action}`, entry); return entry;
        } catch { return null; }
    };
    EAS.Log = { sanitizeUrl, debug: (module, action, data, context) => write('debug', module, action, data, context), info: (module, action, data, context) => write('info', module, action, data, context), warn: (module, action, data, context) => write('warn', module, action, data, context), error: (module, action, error, context = {}) => write('error', module, action, {}, { ...context, error }), entries: ({ module, level, limit = LOG_LIMIT } = {}) => (EAS.Logger ? EAS.Logger.entries().map(e=>({...e, action:e.event, level:e.level.toLowerCase()})) : storageGet(LOG_KEY, [])).filter((entry) => (!module || entry.module === module) && (!level || entry.level === level)).slice(-limit), clear: () => { EAS.Logger?.clear(); storageSet(LOG_KEY, []); }, context: pageContext };
