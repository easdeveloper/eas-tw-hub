(() => {
    ...
    const normalizeVillage = (value = {}, source = value.source || 'unknown') => {
        ...
        return { id, name: rawName || `Aldeia ${parsed.coordinate}`, coord: parsed.coordinate, coordinate: parsed.coordinate, x: parsed.x, y: parsed.y, points: parseNumber(value.points), groupIds: [...new Set((value.groupIds || value.groups || []).map?.((group) => String(group?.id ?? group)) || [])], owned: true, href: value.href || '', source, lastSeenAt: Number(value.lastSeenAt || Date.now()) };
    };
    ...
    const requestAuthoritativeSnapshot = async () => { log('villages-refresh-request', { url: buildOverviewUrl().pathname }); if (window.EASRateLimit?.check()) throw new Error('RATE_LIMITED'); const response = await fetch(buildOverviewUrl(), { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } }); if (window.EASRateLimit?.reportStatus(response.status)) throw Object.assign(new Error('RATE_LIMITED'), { status: 429 });
        if (!response.ok) throw new Error(`Não foi possível atualizar aldeias (HTTP ${response.status}).`); const doc = new DOMParser().parseFromString(await response.text(), 'text/html'); if (doc.querySelector('form#login,input[name="password"]')) throw new Error('Sessão expirada ao atualizar aldeias.'); const villages = readOverviewSnapshot(doc); if (!villages.length) throw new Error('O overview autenticado não retornou aldeias.'); return { villages, source: 'authenticated-overview-request', complete: true }; };
