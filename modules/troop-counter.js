(() => {
    'use strict';
    EAS.Modules ||= {};
    const SETTINGS_KEY = 'troop-counter.settings';
    ...
    const buildExport = ({ groupName, typeLabel, aggregate, generatedAt = new Date() }) => [`Grupo,${csvCell(groupName)}`, `Tipo,${csvCell(typeLabel)}`, `Data/hora,${csvCell(generatedAt.toLocaleString('pt-BR'))}`, `Aldeias,${aggregate.villageCount}`, '', 'Unidade,Total', ...aggregate.units.map((unit) => `${csvCell(UNIT_LABELS[unit] || unit)},${aggregate.totals[unit] || 0}`)].join('\r\n');
    const download = (text, filename = 'contador-de-tropas.csv') => { const url = URL.createObjectURL(new Blob(['\uFEFF', text], { type: 'text/csv;charset=utf-8' })); window.__EASImageTraceMark?.({ event: 'csv-blob-created', asset: filename, url, type: 'text/csv;charset=utf-8', stack: new Error('EAS CSV creation').stack }); const link = document.createElement('a'); link.href = url; link.download = filename; link.click(); setTimeout(() => URL.revokeObjectURL(url), 0); };
    ...
    EAS.Modules.TroopCounter = {
        ...
        open: async () => {
            ...
            try { await EAS.Data.Groups.ensureFresh(); } catch (error) { EAS.Log?.error?.('troop-counter', 'groups-load-failed', error); }
            ...
            const groups = EAS.Data.Groups.getAll();
            ...
            const settings = EAS.Storage?.get?.(SETTINGS_KEY, {}) || {};
            groupSelect.value = EAS.Data.Groups.getById(settings.groupId) ? String(settings.groupId) : 'all';
            if (supported.has(settings.type)) typeSelect.value = settings.type;
            ...
            groupSelect.onchange = async () => { const sequence = ++renderSequence; persistSettings(); status.innerHTML = '<small>Carregando aldeias do grupo…</small>'; try { await EAS.Data.Groups.ensureMembership(groupSelect.value); } catch (error) { EAS.Log?.error?.('troop-counter', 'group-membership-load-failed', error, { groupId: groupSelect.value }); } if (sequence === renderSequence) render(); };
            typeSelect.onchange = () => { persistSettings(); render(); };
            if (groupSelect.value !== 'all') await EAS.Data.Groups.ensureMembership(groupSelect.value).catch((error) => EAS.Log?.error?.('troop-counter', 'group-membership-load-failed', error, { groupId: groupSelect.value }));
            render(); return win;
        }
    };
})();
