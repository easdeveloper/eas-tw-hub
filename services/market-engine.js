(() => {
    ...
    const retryAfterMs = (response) => { const value = response?.headers?.get?.('Retry-After'); if (!value) return 0; const seconds = Number(value); if (Number.isFinite(seconds)) return Math.max(0, seconds * 1000); const timestamp = Date.parse(value); return Number.isFinite(timestamp) ? Math.max(0, timestamp - Date.now()) : 0; };
    const requestWithBackoff = async ({ url, villageId = null, executionId = null, requestType = 'market', module = 'market.balance', maxRetries = 1 } = {}) => { let lastOperations = []; for (let attempt = 1; attempt <= maxRetries + 1; attempt += 1) { const startedAt = Date.now(); lastOperations = [...lastOperations, { attempt, startedAt, endpoint: `${url.pathname}?${url.searchParams}` }].slice(-5); const response = await fetch(url, { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } }); if (response.status !== 429) { if (!response.ok) throw Object.assign(new Error(`HTTP ${response.status}`), { status: response.status, endpoint: url.href }); return response; } const delayMs = retryAfterMs(response) || Math.min(8000, 1000 * (2 ** (attempt - 1))); const detail = { executionId, endpoint: url.href, villageId, attempt, timestamp: Date.now(), module, screen: url.searchParams.get('screen'), mode: url.searchParams.get('mode'), requestType, retryAfter: response.headers?.get?.('Retry-After') || null, delayMs, lastOperations }; EAS.Log?.warn?.('market.balance', 'http429', detail); if (attempt > maxRetries) throw Object.assign(new Error('Limite temporário do servidor. Tente novamente em alguns segundos.'), { status: 429, ...detail }); await wait(delayMs); } };
    ...
    const requestMarketVillage = (village, options = {}) => { const key = String(village?.id ?? village?.villageId ?? ''); if (inFlightVillageRequests.has(key)) return inFlightVillageRequests.get(key); const request = (async () => { const perfMark = EAS.Utils.Perf?.start('market.village-request', { villageId: key }); const url = new URL('/game.php', location.origin); url.searchParams.set('village', key); url.searchParams.set('screen', 'market'); url.searchParams.set('mode', 'own_offer'); const response = await requestWithBackoff({ url, villageId: key, executionId: options.executionId, requestType: 'village-market-state' }); const html = await response.text(); const doc = new DOMParser().parseFromString(html, 'text/html'); const parsed = parseMarketVillageDocument(doc, village); EAS.Utils.Perf?.end(perfMark, { status: parsed.status }); return { parsed, normalized: normalizeVillage({ ...village, ...parsed, activeOffers: parsed.activeOffers, activeOfferList: parsed.activeOfferList, source: 'market-page' }) }; })().finally(() => inFlightVillageRequests.delete(key)); inFlightVillageRequests.set(key, request); return request; };
    ...
    const requestMarketTransports = (villageId, options = {}) => { const key = String(villageId); if (inFlightTransportRequests.has(key)) return inFlightTransportRequests.get(key); const request = (async () => { const mark = EAS.Utils.Perf?.start('balance.transport-scan', { villageId: key }); const url = new URL('/game.php', location.origin); url.searchParams.set('village', key); url.searchParams.set('screen', 'market'); url.searchParams.set('mode', 'transports'); const response = await requestWithBackoff({ url, villageId: key, executionId: options.executionId, requestType: 'market-transports' }); const doc = new DOMParser().parseFromString(await response.text(), 'text/html'); const movements = parseMarketTransportDocument(doc, key); EAS.Utils.Perf?.end(mark, { movementCount: movements.length }); return movements; })().finally(() => inFlightTransportRequests.delete(key)); inFlightTransportRequests.set(key, request); return request; };
    ...
    const refreshMarketVillage = async (villageId, options = {}) => {
        ...
        const previous = cacheVillages(cache).find((item) => String(item.villageId) === key); const { parsed, normalized } = await requestMarketVillage(village, options); normalized.revision = amount(previous?.revision) + 1; normalized.lastMutationSource = 'market-remote-refresh';
