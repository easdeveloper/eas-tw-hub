(() => {
    ...
    // Balanceamento intentionally excludes requested resources from open offers.
    const getConfirmedProjectedResources = (village) => { const item = normalizeVillage(village); return addResources(getAvailableResources(item), item.incomingTransports); };
    ...
    const refreshMarketNetworkState = async ({ villageIds, maxAgeMs = 15000, concurrency = 2 } = {}) => {
        ...
        if (inFlightNetworkRequests.has(key)) return inFlightNetworkRequests.get(key);
        const request = (async () => { console.debug('market-network-scan-start', { villageCount: ids.length }); console.debug('balance-transports-scan-start', { villageCount: ids.length }); const result = []; const movements = []; let cursor = 0;
            const worker = async () => { while (cursor < ids.length) { const id = ids[cursor++]; const currentCache = getCache(); const cached = cacheVillages(currentCache).find((entry) => String(entry.villageId) === id); if (cached && Date.now() - cached.updatedAt <= maxAgeMs) { console.debug('market-network-village-cache-hit', { villageId: id }); result.push(cached); movements.push(...(currentCache.movements?.[id] || [])); continue; } console.debug('market-network-village-request', { villageId: id }); const [village, villageMovements] = await Promise.all([refreshMarketVillage(id), requestMarketTransports(id)]); result.push(village); movements.push(...villageMovements); const updated = getCache(); updated.movements ||= {}; updated.movements[id] = villageMovements; saveCache(updated); } };
            await Promise.all(Array.from({ length: Math.max(1, Math.min(concurrency, ids.length)) }, worker)); const state = createMarketNetworkState({ villages: result, movements }); const updated = getCache(); updated.villages = Array.isArray(updated.villages) ? Object.fromEntries(updated.villages.map((entry) => [String(entry.villageId), entry])) : (updated.villages || {}); state.villages.forEach((entry) => { if (!updated.villages[entry.villageId]) return; updated.villages[entry.villageId].incomingTransports = entry.incoming; updated.villages[entry.villageId].outgoingTransports = { ...entry.outgoing, alreadyDebited: false }; updated.villages[entry.villageId].transportSource = entry.transportSource; }); updated.updatedAt = Date.now(); saveCache(updated); console.debug('balance-transports-scan-complete', { transportCount: state.transports.length }); console.debug('market-network-scan-complete', { villageCount: result.length }); return state;
        })().finally(() => inFlightNetworkRequests.delete(key)); inFlightNetworkRequests.set(key, request); return request;
    };
