(() => {
    ...
    // Keep the public API name for callers; HTTP 429 never authorizes a retry.
    const requestWithBackoff = async ({ url, villageId = null, executionId = null, requestType = 'market' } = {}) => {
        if (window.EASRateLimit?.check()) throw Object.assign(new Error('RATE_LIMITED'), { status: 429 });
        const response = await fetch(url, { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } });
        if (response.status === 429) {
            window.EASRateLimit?.reportStatus(429);
            EAS.Log?.warn?.('market.balance', 'http429', { executionId, villageId, requestType, attempt: 1, retry: false });
            throw Object.assign(new Error('RATE_LIMITED'), { status: 429, executionId, villageId });
        }
        if (!response.ok) throw Object.assign(new Error(`HTTP ${response.status}`), { status: response.status, endpoint: url.href });
        return response;
    };
