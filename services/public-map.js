(() => {
    ...
    const loadOnce = async (forceRefresh = false) => {
        ...
        EAS.Utils.Perf?.end(perfMark, { playerCount: players.length, villageCount: villages.length });
        return cache;
    };
    const load = (forceRefresh = false) => {
        if (!forceRefresh && cache && Date.now() - cache.updatedAt < CACHE_TTL) return Promise.resolve(cache);
